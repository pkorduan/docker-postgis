...
ssh -i /home/gisadmin/.ssh/id_ed25519 gisadmin@ip \
  "docker exec $CONTAINER_ALT pg_dumpall $DUMP_FLAGS --username=postgres --host=localhost" \
  2>"$LOG_DIR/dump_stderr.log" | \
docker exec -i "$CONTAINER_NEU" psql postgres postgres \
  >"$LOG_DIR/restore_stdout.log" \
  2>"$LOG_DIR/restore_stderr.log"
